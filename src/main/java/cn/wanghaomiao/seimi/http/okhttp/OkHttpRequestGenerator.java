...
public class OkHttpRequestGenerator {
    public static Request.Builder getOkHttpRequesBuilder(cn.wanghaomiao.seimi.struct.Request seimiReq, CrawlerModel crawlerModel){
        ...
        if (seimiReq.isUseSeimiAgent()){
            ...
            if(StringUtils.isNotBlank(crawler.cookies())){
            	requestBuilder.header("Cookie", seimiReq.cookies());
            }
            else if(StringUtils.isNotBlank(seimiReq.cookies())){
            	requestBuilder.header("Cookie", seimiReq.cookies());
            }
            ...
            if(StringUtils.isNotBlank(crawler.cookies())){
            	requestBuilder.header("Cookie", crawler.cookies());
            }
            else if(StringUtils.isNotBlank(seimiReq.cookies())){
            	requestBuilder.header("Cookie", seimiReq.cookies());
            }
        ...
        if (!CollectionUtils.isEmpty(seimiReq.getHeader())) {
            for (String headerName : seimiReq.getHeader().keySet()) {
            	requestBuilder.addHeader(headerName, seimiReq.getHeader().get(headerName));
            }
        }
